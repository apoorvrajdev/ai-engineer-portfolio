...
- `next.config.mjs` also sets the security headers, and the CSP there is **same-origin only** — every `*-src` directive is `'self'`, with `data:` allowed for images and `'unsafe-inline'` kept for Next's inline scripts and styles. `next/font` self-hosts Onest and JetBrains Mono under `/_next/static/media`, so no Google Fonts origin is needed. Adding an external font, script, image host or API endpoint means adding its origin to the policy first: a blocked request surfaces only as a console error in the browser, never as an `npm run build` or `npm run lint` failure.
