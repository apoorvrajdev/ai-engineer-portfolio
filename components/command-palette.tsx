...
// Matches the backdrop's `duration-150` fade, so the dialog survives exactly as
// long as the closing transition and no longer.
...
export function CommandPalette() {
  ...
  if (open !== prevOpen) {
    ...
    // Mount the dialog in the same render that opens it, so the backdrop's
    // fade and the contents appear together.
    if (open) setShowContents(true)
  }

  // A reopen inside the closing transition reuses the still-mounted dialog, so
  // `autoFocus` does not fire. Re-focus on every open instead, which also gives
  // cmdk's keyboard navigation the right target.
  useEffect(() => {
    if (!open) return
  ...
  // Nothing focusable may remain inside the backdrop once it is `aria-hidden`,
  // or Tab lands on an invisible input and the focus ring disappears. `inert`
  // covers the closing transition; this unmounts the dialog once it has faded.
  useEffect(() => {
    if (open || !showContents) return
    const id = window.setTimeout(() => setShowContents(false), CLOSE_TRANSITION_MS)
